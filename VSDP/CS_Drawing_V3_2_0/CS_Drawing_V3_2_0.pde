...
void sprayButton(){
  ...
  noFill();
  rect(1100,100,51,51);
  if(mousePressed && mouseX > 1100 && mouseX < 1150 && mouseY > 100 && mouseY < 150){
    tool = 2;
  }
  if(keyPressed && key == 'c'){
    tool = 2;
...
void selectedButton(){
  ...
  fill(0);
  textSize(15);
  text("Current Tool",1085,280);
  if(tool == 0){
    image(pencilButton,1026,251,50,50);
  }
  if(tool == 1){
    image(eraserButton,1026,251,50,50);
  }
  if(tool == 2){
    image(sprayCanButton,1026,251,50,50);
  }
}
...
void currentColorButton(){
  fill(255);
  rect(1025,560,50,50);
  fill(red,green,blue);
  ellipse(1050,585,40,40);
  textSize(15);
  fill(0);
  text("Current Colour",1085, 590);
}
